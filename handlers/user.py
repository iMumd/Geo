...
class UserHandlers:
    ...
    def _register_callbacks(self):
        ...
        @self.app.on_callback_query(filters.regex(r"^(start|help|setlang)_"))
        @handle_errors
...
• `/antiscam` - Toggle anti-scam
• `/antiporn` - Toggle NSFW filter
...
"""
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🛡️ Protection Help", callback_data="help_protection")],
            [InlineKeyboardButton("🌐 Language", callback_data="setlang_main")]
        ])
...
"""
        ...
        if user_info:
            text += f"• Warnings: {user_info['warnings']}\n"
    ...
    async def handle_callback(self, client: Client, callback: CallbackQuery):
        ...
        if action == "setlang":
            ...
            await callback.message.edit_text(
                f"✅ Language set to {translator.get_all_languages().get(data, 'English')}!"
            )
        
        elif action == "help":
            await callback.answer()
            # Update help message based on callback data
